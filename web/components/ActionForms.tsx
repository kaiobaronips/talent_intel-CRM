  ...
  return (
    <label className="grid gap-2 text-sm font-bold text-stone-700">
      ...
      <textarea
        ...
        rows={3}
        placeholder={placeholder}
        className="resize-none rounded-lg border border-stone-200 bg-white px-4 py-3 font-medium text-stone-950 outline-none transition placeholder:text-stone-400 focus:border-amber-500 focus:ring-4 focus:ring-amber-100"
      />
    </label>
  );
}
...
export function HunterEnrichmentForm({ tenantId }: { tenantId: string }) {
  const [state, action] = useActionState(runHunterEnrichmentAction, initialState);
  ...
  return (
    <form action={action} className="rounded-lg border border-stone-200 bg-white p-5 shadow-sm">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <p className="text-xs font-bold uppercase text-amber-700">Enriquecimento de contato</p>
          <h2 className="mt-2 font-display text-2xl font-black">Encontrar e validar e-mails no Hunter.io</h2>
          <p className="mt-2 max-w-3xl text-sm font-medium leading-6 text-stone-600">
            Use depois da busca no Apollo. O Hunter procura e-mails profissionais e só libera o candidato para os agentes quando existe canal de contato válido.
          </p>
        </div>
        <SubmitButton>Rodar Hunter</SubmitButton>
      </div>
