...
import { interactionDisplayStatus, providerConfirmationLabel } from '@/lib/interactions';
...
function InteractionContactCard({
  interaction,
  tenantId,
  showCandidateLink,
}: {
  ...
  const displayStatus = interactionDisplayStatus(interaction);
  const providerConfirmation = providerConfirmationLabel(interaction);
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            <StatusBadge value={displayStatus.value} label={displayStatus.label} />
          </div>
          {showCandidateLink ? (
      ...
      <div className="mt-4 grid gap-3">
        ...
        {providerConfirmation ? (
          <div className="rounded-lg bg-sky-50 p-4">
            <p className="text-xs font-bold uppercase text-sky-700">Status do provedor</p>
            <p className="mt-2 break-words text-sm font-bold text-sky-950">{providerConfirmation}</p>
          </div>
        ) : null}
      </div>
      ...
      {interactions.length === 0 ? (
        ...
        <div className="mt-5 grid gap-4 xl:grid-cols-2">
          {interactions.map((interaction) => (
            <InteractionContactCard key={interaction.id} interaction={interaction} tenantId={tenantId} showCandidateLink={showCandidateLink} />
          ))}
        </div>
      )}
    </section>
